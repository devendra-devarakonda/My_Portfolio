...
void mainImage(out vec4 fc,in vec2 frag){
    ...
    vec3 midColor = vec3(1.0, 0.35, 0.48); // #FF5A7A
    vec3 lightColor = vec3(1.0, 0.58, 0.66); // #FF95A8
    vec3 colBlend = baseColor;
    if (tone > 0.0) {
        colBlend = mix(baseColor, midColor, clamp(tone * 0.5, 0.0, 1.0));
        colBlend = mix(colBlend, lightColor, clamp((tone - 1.0) * 0.3, 0.0, 1.0));
        colBlend = mix(colBlend, vec3(1.0, 1.0, 1.0), clamp((tone - 2.5) * 0.2, 0.0, 1.0));
    }
    vec3 col = colBlend * tone + dith;
...
export const LaserFlow: React.FC<Props> = ({
  ...
  // Sync state refs for rendering loop checks
  ...
  useEffect(() => {
    ...
    const adjustDprIfNeeded = (now: number, frameCost: number) => {
      ...
      // GPU performance safety throttles
      if (avgFps < 50) {
        fpsDegradedRef.current = true;
        next = Math.min(0.75, base);
        if (Math.abs(next - currentDprRef.current) > 0.01) {
          currentDprRef.current = next;
          setSizeNow();
        }
        console.warn(`LaserFlow: FPS dropped below 50 (${avgFps.toFixed(1)}). Degrading quality.`);
      } else if (avgFps < 55 || frameCost > 8.0) {
        fpsDegradedRef.current = true;
        next = Math.min(0.75, base);
        if (Math.abs(next - currentDprRef.current) > 0.01) {
          currentDprRef.current = next;
        ...
        console.warn(`LaserFlow: Running degraded (FPS: ${avgFps.toFixed(1)}, Cost: ${frameCost.toFixed(2)}ms). Disabling wisps and fog.`);
    ...
    const animate = () => {
      ...
      if (pausedRef.current || !inViewRef.current || !visibleRef.current) return;
      ...
      // Adapt uniforms based on degradation state
      if (degradedRef.current || fpsDegradedRef.current) {
        uniforms.uWispDensity.value = 0.6;
        uniforms.uWIntensity.value = 4.0;
        uniforms.uFogIntensity.value = 0.3;
      } else {
      ...
      // Measure render frame cost (GPU Budget checking)
      const tStart = performance.now();
      renderer.render(scene, camera);
      const frameCost = performance.now() - tStart;

      adjustDprIfNeeded(performance.now(), frameCost);
    };
